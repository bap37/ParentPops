...
class Optimizer_Calculation:
    ...
    def optimize_secondary(self, Param, dfdata, SHAPE2, dfpre, dfpost, binsize, SHAPE, MASS):
        dfk = dfdata
        if MASS != None:
            window = 0.6
            dfk = dfk.loc[(dfk.HOST_LOGMASS < MASS +window) & (dfk.HOST_LOGMASS >= MASS - window)]
    ...
    def optimize_in_range(self, Param, dfdata, DIM, dfpre, dfpost, binsize, MASS, Shape):
        mean = []
        stdl= []
        stdr = []
        n = []
    ...
    def optimize_secondary_in_range(self, Param, dfdata, SHAPE2, dfpre, dfpost, binsize, SHAPE):
        collected_result = []
        for i in SHAPE2:
            collected_result.append([])
        
        for m in np.arange(6, 14, .2):
            calculation = self.optimize_secondary(Param, dfdata, SHAPE2, dfpre, dfpost, binsize, SHAPE, m)
